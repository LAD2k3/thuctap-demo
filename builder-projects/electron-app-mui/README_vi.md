...
Tài liệu này cung cấp hướng dẫn toàn diện về kiến trúc, cấu trúc code, quy trình phát triển và cách mở rộng ứng dụng với các trò chơi mới.
...
Đây là tác vụ mở rộng phổ biến nhất. Hãy làm theo các bước cẩn thận.

### Bước 1: Tạo Game Template

Tạo một folder mới trong `template-projects/`:

```bash
cd template-projects
cp -r group-sort my-new-game  # hoặc scaffold từ đầu
```

**Yêu cầu template**:
- Sử dụng `vite-plugin-singlefile` để tạo ra một `index.html` duy nhất
- Đọc `window.APP_DATA` tại runtime
- Đặt tài sản hình ảnh trong folder `images/`
- Thêm `meta.json` với metadata của template
...
Tùy chọn thêm `thumbnail.png` cho thẻ trên màn hình chính.

### Bước 2: Đăng ký trong Script Build

Chỉnh sửa `build-templates.sh`:

```bash
GAMES=(
  "template-projects/group-sort|group-sort"
  "template-projects/my-new-game|my-new-game"  # ← Thêm dòng này
)
```
...
### Bước 3: Đăng ký trong CI Workflow

Chỉnh sửa `.github/workflows/build-all.yml`:
